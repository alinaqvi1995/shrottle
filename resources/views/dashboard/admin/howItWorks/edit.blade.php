...
@section('content')
    <div class="container">
        ...
        <form method="post" action="{{ route('admin.how.update') }}" enctype="multipart/form-data">
            @csrf
            @method('post')
            <input type="hidden" name="faq_id" value="{{ $how->id }}">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="question" class="form-label fs-14 text-theme-primary fw-bold">Question</label>
                    <input type="text" class="form-control fs-14 h-50px" name="question" value="{{ $how->question }}">
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label for="answer" class="form-label fs-14 text-theme-primary fw-bold">Answer</label>
                    <input type="text" class="form-control fs-14 h-50px" name="answer" value="{{ $how->answer }}">
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select name="status" id="role" class="form-control fs-14  h-50px" required>
                        <option @if($how->status == 1) selected @endif value="1">Active</option>
                        <option @if($how->status == 0) selected @endif value="0">Inactive</option>
                    </select>
                </div>
            </div>
